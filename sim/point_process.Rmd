...
```{r}
result <- sim_wrapper(nu, alpha, beta_r, nu_r, alpha_r, tau, lambdas)
marks <- result$marks %>%
  mutate(variable = paste0("X", mark))
...
In particular, we'll make up this equation, for some hypothetical outcome y,
 ...
 1y =  (beta_r[1] - beta_r[2] - beta_r[3]) / 3 + (nu_r + alpha_r + nu + alpha) / 4 - tau + (lambdas[1] - 50) / 100 + noise
...
eps <- 0.1
X <- cbind(beta_r / 3, nu_r / 4, alpha_r / 4, nu / 4, alpha / 4, tau, (lambdas[, 1] - 50) / 800)
y <- X %*% c(1, -1, -1, 1, 1, 1, 1, -1, 1) + rnorm(n_images, 0, eps)
...
for (i in seq_len(n_images)) {
  result <- sim_wrapper(nu[i], alpha[i], beta_r[i, ], nu_r[i], alpha_r[i],
                        tau[i], lambdas[i, ])
...
r <- list()
for (i in seq_along(pts)) {
  r[[i]] <- raster(pts[[i]], ncols=5000, nrows=5000, ext=extent(c(0, 1, 0, 1)))
  r[[i]] <- rasterize(pts[[i]], r[[i]], field = 1, background = 0)
}

writeRaster(stack(r), "data/test2.tiff", overwrite=TRUE)
```
