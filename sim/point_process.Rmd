...
n_original <- 5000 # number of points
beta_r <- c(-0.5, 0, 0.5)
...
lambdas <- c(200, 400, 800) # rate parameters in gamma controlling size
```
...
```{r}
n_images <- 20
n_original <- rnegbin(n_images, 5000, 10)
...
for (j in seq_along(marks)) {
  ...
  r <- list()
  for (i in seq_along(pts)) {
    r[[i]] <- raster(pts[[i]], ncols=500, nrows=500, ext=extent(c(0, 1, 0, 1)))
...
for (i in seq_along(r)) {
  writeRaster(stack(r[[i]]), sprintf("data/image-%s.tiff", i), overwrite=TRUE)
}
