...
lambdas <- runif(n_images * 3, 50, 150) %>%
  matrix(ncol = 3)

# simulate the response
eps <- 0.1
X <- cbind(beta_r / 3, nu_r / 4, alpha_r / 4, nu / 4, alpha / 4, tau, (lambdas[, 1] - 50) / 100)
...
pts <- vector("list", nrow(marks))

for (i in seq_len(nrow(marks))) {
  pt <- as.numeric(marks[i, c("X1", "X2")])
  pts[i] <- st_point(pt) %>%
    st_buffer(marks[i, "size"])
}
