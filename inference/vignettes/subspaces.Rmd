...
train_list <- x_list[grepl("train", names(x_list))]
cca_res <- MultiCCA(train_list, ncomponents = params$ncomp)
```
...
```{r stability_plot, fig.height = 4, fig.width = 12}
pi_hat <- pi_hat %>%
...
ggplot(mcor) +
  geom_text(aes(x = value, y = selection, label = dim, col = split)) +
  scale_color_brewer(palette = "Set2") +
  facet_wrap(~ feature)

ggplot(mcor) +
  geom_point(aes(x = feature, y = value, col = selection)) +
