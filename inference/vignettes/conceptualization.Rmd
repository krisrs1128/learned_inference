...
for (b in seq_len(B)) {
  Pi_hats_[[b]] <- stability_selection(L_aligned$x_align[,, b], y, 1, fit$lambda)[[2]]
}
...
response? The approach below does not do any sample splitting.

```{r}
terms <- factor_model(N, D, K)
response <- simulate_response(terms$L, K) # all betas are 0
detach("terms"); detach("response")

xi <- supervised_svd_extractor(Z, y, K)
Pi_hat <- stability_selection(xi(Z), y, B, fit$lambda)
plot_pi_hat(Pi_hat)
```

With sample splitting, but no feature re-learning.

```{r}
train_ix <- sample(N, N / 2)
xi <- supervised_svd_extractor(Z[train_ix, ], y[train_ix], K)
#xi <- supervised_sca_extractor(Z[train_ix, ], y[train_ix], K)
Pi_hat <- stability_selection(xi(Z[-train_ix, ]), y[-train_ix], B, fit$lambda)
plot_pi_hat(Pi_hat)
```

With feature relearning and sample splitting.
...
for (b in seq_len(B)) {
  ...
  keep_ix <- which(abs(C) > quantile(abs(C), 1 - 2 * K / D))
  sv_z <- svd(Z[, keep_ix])
...
for quite a while. Maybe more importantly, no individual features consistently
stand out from the rest.

```{r}
plot_pi_hats(Pi_hats)
```
