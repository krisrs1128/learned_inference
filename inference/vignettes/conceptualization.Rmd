...
Rscript -e "rmarkdown::render('conceptualization.Rmd', params = list(supervised = F))"

Rscript -e "rmarkdown::render('conceptualization.Rmd', params = list(supervised = T))"
...
response <- simulate_response(terms$L, S)
xi <- svd_projector(Z, K)
Pi_hats[["unadjusted"]] <- stability_selection(xi(Z), y, B, fit$lambda)
```
...
xi <- svd_projector(Z, K)
Pi_hats[["split"]] <- stability_selection(xi(Z), y[-train_ix], B, fit$lambda)
```
