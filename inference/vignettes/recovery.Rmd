...
z <- read_csv(file.path(params$data_dir, conf$organization$features_dir, "features_None.csv")) %>%
  select(-X1) %>%
...
plot(coords, xlim = c(-5, 4.5), asp = 1)
...
ggplot(coords_df) +
  ...
  coord_fixed() +
  labs(
    x = "First MDS Coordinate",
    y = "Second MDS Coordinate"
  )
```

Plot images along the MDS coordinates.
```{r}
xgrid <- seq(-5, 4.5, by=0.5)
...
png("~/Desktop/imgs/encodings_imgs.png", width = 1500, height = 1500, bg="transparent")
plot(xgrid, t="n", asp = 1, xlab = "First MDS Coordinate", ylab = "Second MDS Coordinate")
for (i in seq_len(nrow(xgrid))) {
