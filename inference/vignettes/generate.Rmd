...
params:
  out_dir: /Users/kris/Documents/stability_data_big/
---
...
for (i in seq_len(n_images)) {
  ...
  paths[i] <- sprintf(file.path(tiles_dir, "image-%s.tiff"), str_pad(i, 4, "left", 0))
  save_raster(marks, paths[i])
}

write_csv(data.frame(paths = paths, X = X, y = y), file.path(params$out_dir, "Xy.csv"))
```
