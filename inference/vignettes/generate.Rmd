...
write_csv(data.frame(X = X, y = y), file.path(params$out_dir, "Xy.csv"))
```

Now we can simulate all the images.

```{r}
dir.create(file.path(params$out_dir, "tiffs"))
...
for (i in seq_len(n_images)) {
  ...
  out_name <- sprintf(file.path(params$out_dir, "tiffs", "image-%s.tiff"), str_pad(i, 4, "left", 0))
