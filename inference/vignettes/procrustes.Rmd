...
```{r parse_metadata}
metadata <- map(output_dirs, ~ cbind(abs_path = .x, read_csv(file.path(.x, "metadata.csv")))) %>%
  map_dfr(~ .x %>% 
      mutate(
...
```{r, correlation_data, eval = FALSE, fig.width = 8, fig.height = 5}
lambda_thr <- lambda[8]
...
```{r, correlation_diagram, eval = FALSE, fig.width = 8, fig.height = 5}
