...
save_dir <- file.path(params$save_dir, model_prefix)
...
ggsave(file.path(save_dir, sprintf("embeddings_%s.png", is_sca)))
```
```{r}
...
write_csv(edge_dists, file.path(save_dir, sprintf("edge_dists_%s.csv", params$model_prefix, sca_label)))
```
...
for (b in seq_along(xy)) {
  ...
  png(file.path(save_dir, sprintf("y_vs_y_hat-%s-%s.png", names(xy)[b], save_label)))
