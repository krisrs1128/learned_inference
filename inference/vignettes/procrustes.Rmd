...
ggsave(sprintf("embeddings_%s_%s.png", params$model_prefix, params$sca))
```
```{r}
...
write_csv(edge_dists, file.path(data_dir, sprintf("edge_dists_%s_%s.csv", params$model_prefix, params$sca)))
```
...
for (b in seq_along(xy)) {
  ...
  png(sprintf("y_vs_y_hat-%s-%s-%s.png", params$model_prefix, names(xy)[b], params$sca))
...
ggplot(pi_hat %>% filter(j < 17)) +
  ...
  facet_wrap(~ j, ncol = 6) +
  theme(legend.position = "bottom")
ggsave(sprintf("selection_paths_%s_%s.png", params$model_prefix, params$sca))
```
...
write_csv(cca_data, file.path(data_dir, sprintf("cca_data_%s_%s.csv", params$model_prefix, params$sca)))
```
...
ggsave(sprintf("%s_%s_correlation_diagram.png", params$model_prefix, params$sca))
```
```{r correlation_heatmap}
scores_mat <- scores %>%
  select(starts_with("dim"), starts_with("X")) %>%
  as.matrix()
png(sprintf("%s_%s_correlation_heatmap.png", params$model_prefix, params$sca))
...
write_csv(selections, file.path(data_dir, sprintf("selections_%s_%s.csv", params$model_prefix, params$sca)))
```
```{r remove_unzipped}
paths0 <- gsub(".gz", "", paths)
unlink(paths0, recursive = TRUE)
```
