...
params:
  input_model: "data/cnn_data/cnn-k64.yaml_0.tar.gz"
...
```{r}
attach(params)
exdir <- str_remove(input_model, ".tar.gz")
untar(input_model, exdir = exdir)
```

```{r}
z_path <- list.files(exdir, layer_prefix, recursive = T, full = T)
f <- ifelse(transform == "log", function(x) log(1 + x), identity)
Zb <- f(drop(np$load(z_path)))
Xy <- read_csv(list.files(exdir, "Xy.csv", recursive = T, full = T))
```

```{r}
ix <- list.files(exdir, "*subset*", recursive = TRUE, full = T) %>%
  read_csv() %>%
  rename(ix = X1) %>%
  left_join(Xy)
```
