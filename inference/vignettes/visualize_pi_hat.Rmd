...
plot_curves <- function(pi_hat, sca_bit, perc) {
  ggplot(pi_hat %>% filter(j > 0, sca == sca_bit, train_perc == perc)) +
    geom_line(
      aes(x = log(lambda), 
          y = value, 
          col = split, 
...
for (i in seq_along(p)) {
  ggsave(
    sprintf("~/Desktop/conceptual/learned_inference/notes/figure/selection_paths-%s.png", i), 
    p[[i]],
    dpi = 350,
    height = 4,
    width = 12
