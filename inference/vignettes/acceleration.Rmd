...
```{r}
features_dirs <- list(
  "bootstraps" = file.path(params$data_dir, "features", "vae_no_boot"),
  "start_10" = file.path(params$data_dir, "features", "vae_tune_10"),
  "start_50" = file.path(params$data_dir, "features", "vae_tune_50")
)
...
for (i in seq_along(features_dirs)) {
  ...
  for (p in features_paths) {
    features_list[[cur_dir]][[p]] <- read_csv(p) %>%
      select(-X1) %>%
      select(path, model, img_id, y, everything())
...
for (i in seq_along(tune_features)) {
  ...
  for (j in seq_along(features_list[[ft]])) {
    features_list[[ft]][[j]] <- features_list[[ft]][[j]] %>%
      filter(!grepl("boot", model))
  }

  ## features_list[[ft]] <- bind_rows(features_list[[ft]], id. = "boot")
}
