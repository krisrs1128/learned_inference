...
```{r, correlation_data, fig.width = 8, fig.height = 5, eval = FALSE}
lambda_thr <- lambda[8]
...
for (j1 in seq_len(params$ncomp)) {
  for (j2 in seq_len(j1 - 1)) {
    ...
    image_grid(coords, paths, min_dist = .4, density = 10, imsize = 2) +
      labs(
...
coords <- centroids %>%
  ...
  select(one_of(cur_cols))
colnames(coords) <- c("x", "y")
image_grid(coords, paths, min_dist = .4, density = 20, imsize = .4) +
  labs(
    x = sprintf("Learned Feature %s", j1),
    y = sprintf("Learned Feature %s", j2)
  )
ggsave("~/Desktop/tmp.png", dpi = 350)
```
