...
```{r, embedding_plot_density, fig.width = 8}
density_data <- plot_data %>% 
  dcast(rel_path + y + split + bootstrap ~ dim, value.var = "value") %>%
  split(.$split) %>%
  map_dfr(~ head(.x, 150))
density_data$rel_path <- as.numeric(as.factor(density_data$rel_path))

ggplot(density_data) +
  stat_density_2d(
    aes(x = dim1, y = dim2, group = rel_path, fill = y),
    geom = "polygon",
    alpha = 0.1,
    binwidth = 0.2
  ) +
...
for (j1 in seq_len(params$ncomp)) {
  for (j2 in seq_len(j1 - 1)) {
    ...
    image_grid(coords, paths, min_dist = .05, density = 10, channels = c(4, 5, 7))
