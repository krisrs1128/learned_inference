---
title: Stability study
---


```{r}
library("FactoMineR")
library("PMA")
library("dplyr")
library("ggplot2")
library("readr")
library("stringr")
library("yaml")
library("inference")
```

Let's read in all the bootstrapped learned features. We save the full feature
data frame in `features_list` and the purely numerical component in `z`.

```{r}
data_dir <- file.path("~/Desktop", "data")
root_dir <- file.path("..", "..")
conf <- yaml.load_file(file.path(root_dir, "conf", "train.yaml"))
features_dir <- file.path(data_dir, conf$organization$features_dir)
features_paths <- list.files(features_dir, full.names = TRUE)

features_list <- list()
for (p in features_paths) {
  features_list[[p]] <- read_csv(p) %>%
    select(-X1) %>%
    select(path, y, everything())
  features_list[[p]]$ix <- str_extract(features_list[[p]]$path, "[0-9]+") %>%
    as.integer()
}

Xy <- read_csv(file.path(data_dir, conf$organization$xy))

z <- lapply(features_list, function(z) as.matrix(z[, 3:66]))
heatmap(cor(Xy[features_list[[1]]$ix, ], z[[1]]))
hist(cor(features_list[[1]]$y, z[[1]]), 20)
```

For our first summary, we'll calculate the RV coefficients between unprojected
pairs. I'm not sure if this is supposed to have any meaning if the columns are
not exactly aligned. It's just a measure of the overall information content
shared between the pairs, regardless of the column that is the source.

```{r}
rvs <- pairwise_rv(z)
heatmap(rvs)
hist(rvs, 20)
```

Calculate pairwise RV coefficients between the projected pairs. Note an
interesting fact: when we ust multicca on noise, the projected scores will
appear correlated (because, of course, we optimized that).

```{r}
n_comp <- 10
cca_res <- PMA::MultiCCA(features_list, ncomponents = n_comp)

scores <- list()
for (b in seq_len(B)) {
  scores[[b]] <- features_list[[b]] %*% cca_res$ws[[b]]
}

rvs_aligned <- pairwise_rv(scores)
image(rvs_aligned)
hist(rvs_aligned, 20)
```

We can also try some dummy stability selection. This will depend on the scores.

```{r}
y <- rnorm(n)
library("glmnet")

fits <- list()
coefs <- list()
i <- 1
for (b in seq_len(B)) {
  fits[[b]] <- glmnet(scores[[b]], y)
  for (j in seq_len(n_comp + 1)) {
    coefs[[i]] <- data.frame(
      lambda = fits[[b]]$lambda,
      j = j,
      beta_j = coef(fits[[b]])[j, ]
    )
    i <- i + 1
  }
}

coefs <- bind_rows(coefs, .id = "b")
rownames(coefs) <- NULL

ggplot(coefs) +
  geom_line(aes(x = lambda, y = beta_j, group = b)) +
  facet_wrap(~ j) +
  ggtitle("Projected Coefs. across Bootstraps")
```
