...
dpath <- function(x, s) {
  read_csv(file.path(x, params$model_prefix, paste0(s, collapse = "/")))
}
```
...
metadata <- map(output_dirs, ~ dpath(.x, "metadata.csv"))
...
```{r parse_metadata}
metadata <- map(output_dirs, ~ cbind(abs_path = .x, dpath(.x, "metadata.csv"))) %>%
  map_dfr(~ .x %>%
      mutate(
...
if (params$sca) {
  ...
  sparse_approx <- map(x_list, ~ princomp(.))
}
```
...
feature_stability <- map(scores, ~ sum( (.x - M) ^ 2)) %>%
  unlist() %>%
  mean()
fss <- data.frame(
...
# Xy.csv has different information in TNBC and Simulation splits
scores <- map2_dfr(acts_splits, scores, ~ cbind(.x %>% select(-starts_with("X")), .y))
if (!grepl("tnbc", params$model_prefix)) { 
  scores <- scores %>%
...
for (filt_rows in c(TRUE, FALSE)) {
  if (filt_rows) {
    edge_data_sub <- edge_data %>%
      split(.$split) %>%
  ...
  for (j1 in seq_len(params$ncomp)) {
    for (j2 in seq_len(j1 - 1)) {
      ggplot(edge_data_sub) +
        geom_point(
          data = centroids %>% filter(rel_path %in% edge_data_sub$rel_path),
      ...
      ggsave(file.path(save_dir, sprintf("embeddings_%s-%s-%s_%s.png", sca_label, j1, j2, filt_rows)))
    }
  }
}
```
...
```{r}
paths <- file.path(params$tiles_dir, centroids$rel_path)
for (j1 in seq_len(params$ncomp)) {
  for (j2 in seq_len(j1 - 1)) {
    ...
    image_grid(coords, paths, min_dist = .05, density = 10, channels = params$channels) +
      labs(
        x = sprintf("Learned Feature %s", j1),
        y = sprintf("Learned Feature %s", j2)
      )
    ggsave(file.path(save_dir, sprintf("image_grid-%s-%s-%s.png", sca_label, j1, j2)))
  }
}
```
...
```{r}
save.image(file.path(save_dir, sprintf("stability_%s.RData", sca_label)))
```